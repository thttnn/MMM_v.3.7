...
5-Sales equals Production that equals demand. There is no Delivery Delay initially. 
...
CYCLE(cur, "SECTORS")
{
	...
	v[15]=V("markup_period");
	v[18]=V("rnd_revenue_proportion");
	v[19]=V("indirect_tax_rate");
	v[23]=V("profits_distribution_rate");
	v[24]=V("initial_quality");

	//Begin Wirtting Sector Independent Variables
	for (v[120]=1 ; v[120]<=v[0] ; v[120]=v[120]+1)
		{
		WRITELLS(cur, "Sector_Demand_Met", 1, 0, v[120]);                              				//it is assumed that all demand is met initially. Equals 1 by definition
		WRITELLS(cur, "Sector_Demand_Met_By_Imports", 1, 0, v[120]);                      			//it is assumed thatt all imports are met initially. Equals 1 by definition
		}
	...
	//Begin Writting Sector Dependent Variables
		...
		WRITELLS(cur, "Sector_Max_Quality", v[24], 0,  1);
	...
	for (v[119]=1 ; v[119]<=(v[5]+1) ; v[119]=v[119]+1)                        		 				//for 8 lags
		WRITELLS(cur, "Sector_Avg_Quality", v[24], 0, v[119]);               				//Effective_Orders_Sectors equals demand_initial
	...
	cur1=SEARCHS(cur, "FIRMS");																	//search the first and only instance of firms below each sector
	  ...
	  for (v[102]=1 ; v[102]<=v[0] ; v[102]=v[102]+1)                                			//for 6 lags
	  	{
	  ...
	  for (v[107]=1 ; v[107]<=(v[15]-1) ; v[107]=v[107]+1)												//for 7 lags
	  ...
	  for (v[108]=1 ; v[108]<=v[0] ; v[108]=v[108]+1)                                			//for 6 lags
	  	{
		WRITELLS(cur1, "Firm_Demand_Capital_Goods", 0, 0, v[108]);
	  ...
	  for (v[111]=1 ; v[111]<=(v[15]-1) ; v[111]=v[111]+1)                                				//for 7 lags
	  	WRITELLS(cur1, "Firm_Potential_Markup", v[7], 0, v[111]);                      			//potential markup will be the initial markup
	  for (v[112]=1 ; v[112]<=(2*v[0]) ; v[112]=v[112]+1)										//for 12 lags
...
CYCLE(cur, "CLASSES")
{
	...
	v[39]=v[39]+v[34];																			//total taxes payed by classes
		for (v[113]=1 ; v[113]<=v[5] ; v[113]=v[113]+1)                          					//for 4 lags
			{
...
CYCLE(cur, "MACRO")
{
		...
		WRITELLS(cur,"Annual_Growth", 0, 0, 1);
