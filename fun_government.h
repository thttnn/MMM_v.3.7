...
EQUATION("Government_Desired_RND")
/*
RND expenses.
If there are no maximum expenses, it is adjusted by average productivity growth and inflation.
*/
...
v[0]=V("government_period");
v[1]= fmod((double) t,v[0]);                                   //divides the time period by government adjustment period (adjust annualy)
v[2]=VL("Government_Desired_RND",1);
if(v[1]==0)                                                    //if the rest of the division is zero (adjust government wages)
{
	v[3]=VL("Avg_Productivity",1);                             //avg productivity lagged 1
	v[4]=VL("Avg_Productivity", v[0]);                         //avg productivity lagged government period (4)
	...
	if(v[7]!=0)                                                //if consumer price index is not zero
		...
		v[8]=0;												   //use 1
	v[9]=v[2]*(1+v[5]+v[8]);                                   //desired adjusted government wages with no restriction
}
else                                                           //if it is not adjustment period
	v[9]=v[2];                                                 //use last period's
RESULT(v[9])
...
if(v[1]==0)                                                    //if the rest of the division is zero (adjust unemployment benefits)
{
	v[3]=V("government_investment_growth");
...
if(v[1]==0)                                                    //if the rest of the division is zero (adjust unemployment benefits)
{
	v[3]=V("government_consumption_growth");
	v[5]=VL("Consumer_Price_Index",1);                         //consumer price index lagged 1
	v[6]=VL("Consumer_Price_Index", v[0]);                     //consumer price index lagged government period (4)
	if(v[6]!=0)                                                //if consumer price index is not zero
		v[7]=(v[5]-v[6])/v[6];                                 //calculate inflation
...
if(v[1]==0)                                                    //if the rest of the division is zero (adjust unemployment benefits)
{
	v[3]=V("government_intermediate_growth");
	v[5]=VL("Consumer_Price_Index",1);                         //consumer price index lagged 1
	v[6]=VL("Consumer_Price_Index", v[0]);                     //consumer price index lagged government period (4)
	if(v[6]!=0)                                                //if consumer price index is not zero
		v[7]=(v[5]-v[6])/v[6];                                 //calculate inflation
...
3--> Government Wages(65%) + Unemployment Benefits(0%) + Government Consumption, Investment and Intermediate Demand(10% each) + R&D Expenses [Public Workers, Unemployment Benefits, Demand to Sectors and R&D]
...
if(v[0]==-1)                                                   //no fiscal rule
{
	if(v[7]==0)												   //if only government wages
	{
		...
		v[13]=0;                                               //government RND equals 0
	}
	...
	if(v[7]==3)                                                //if wages, unemployment benefits, consumption and investment
	{
		v[8]=v[1];											   //government wages equal desired wages
		v[9]=v[2];    										   //government unemployment benefits equal 0
		v[10]=v[3];                                            //government consumption equal desired
		v[11]=v[4];                                            //government investment equal desired
		v[12]=v[5];                                            //government intermediate demand equals desired
		v[13]=v[6];                                            //government RND equals desired
	}
}
else
{
	...
	{
		v[8]=min(v[0],v[1]);								   //government wages is desired limited by maximum expenses
		v[9]=min(v[2],(v[0]-v[6]));    						   //government unemployment benefits is desired limited by maximum expenses minus wages
		v[10]=min(v[3],(v[0]-v[8]-v[9]));                      //government consumption is desired limited by maximum expenses minus wages and benefits
		v[11]=min(v[4],(v[0]-v[8]-v[9]-v[10]));                //government investment is desired limited by maximum expenses minus wages and benefits
		v[12]=min(v[5],(v[0]-v[8]-v[9]-v[10]-v[11]));          //government intermediate is desired limited by maximum expenses minus wages and benefits
		v[13]=min(v[6],(v[0]-v[8]-v[9]-v[10]-v[11]-v[12]));    //government RND is desired limited by maximum expenses minus wages and benefits and sectoral demand
	}
}
...
WRITE("Government_Effective_RND", v[13]);
v[14]=v[8]+v[9]+v[10]+v[11]+v[12]+v[13];
RESULT(v[14])
...
EQUATION_DUMMY("Government_Effective_RND","Government_Effective_Expenses")
