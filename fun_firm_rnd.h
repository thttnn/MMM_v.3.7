...
EQUATION("Sector_Tech_Opportunity")
/*
Sectorial Variable.
Fixed if no government RND. Increases otherwise.
*/
v[1]=V("Government_Effective_RND");
v[2]=VL("Government_Effective_RND",1);
if(v[2]!=0)
	v[3]=(v[1]-v[2])/v[2];
else
	v[3]=0;
v[4]=V("government_rnd_adjustment");
v[5]=VL("Sector_Tech_Opportunity",1);
v[6]=v[5]*(1+v[4]*v[3]);
RESULT(v[6])
