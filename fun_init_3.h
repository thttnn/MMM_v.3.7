...
v[41]=V("number_object_firms");																	//sector control parameter
v[42]=V("number_object_firms");																	//sector control parameter
v[43]=V("number_object_firms");																	//sector control parameter

v[44]=V("initial_productivity");																//sector control parameter
v[45]=V("initial_productivity");																//sector control parameter
v[46]=V("initial_productivity");																//sector control parameter
...
v[50]=V("initial_wage");																		//sector control parameter
v[51]=V("initial_wage");																		//sector control parameter
v[52]=V("initial_wage");																		//sector control parameter

v[53]=V("input_tech_coefficient");																//sector control parameter
v[54]=V("input_tech_coefficient");																//sector control parameter
v[55]=V("input_tech_coefficient");																//sector control parameter
...
v[62]=V("profits_distribution_rate");															//sector control parameter
v[63]=V("profits_distribution_rate");															//sector control parameter
v[64]=V("profits_distribution_rate");															//sector control parameter
...
CYCLE(cur, "CLASSES")
{
	...
	v[165]=v[41]*v[161];																		//class initial autonomous consumption
...
CYCLE(cur, "SECTORS")
{
	...
	for (i=1 ; i<=(v[2]+1) ; i++)                        		 								//for (class_period+1) lags
		...
		WRITES(cur, "capital_output_ratio", (v[212]/v[222]));
	...
	for (i=1 ; i<=(v[1]-1) ; i++)																//for (markup_period-1) lags
		...
		WRITELLS(cur1, "Firm_Effective_Market_Share", (1/v[212]), 0, 1);                    	//firm's effective market share will be the initial market share       
	  	...
	  	WRITELLS(cur1, "Firm_Productive_Capacity", (v[222]/v[212]), 0, 1);						//All firms start operating at desired degree of utilization, thus, productive capacity is endogenous calculated based on effective production and desired degree
		WRITELLS(cur1, "Firm_Capital", v[102], 0, 1);											//firm nominal capital equals number of capital if capital goods price equals 1
		...
		WRITELLS(cur1, "Firm_Stock_Deposits", 0, 0, 1);											//no financial assets initially
	  	...
	  	WRITELLS(cur1, "Firm_Avg_Debt_Rate", 0, 0, 1);                       					//no debt initially
			...
			WRITES(cur2, "capital_good_productive_capacity", (v[222]/v[212]));     				//inverse of capital output ratio  
			...
			WRITES(cur2, "id_capital_good_number", 1);                       					//1 for the first and only initial firm (the others will be created below)
	 		WRITES(cur2, "capital_good_date_birth", 0);											//write the capital good date of birth
			WRITES(cur2, "capital_good_depreciation_period", v[5]);
