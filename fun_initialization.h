...
v[76]=(1+v[17])*v[12]/v[18];																	//demand to capitalgoods multiplier
v[19]=(v[8]*v[90]/v[7])/(1-v[8]*v[11]);															//nominal wage
v[31]=v[90]/v[7] + v[13]*(1-v[15])*v[19];																//sector effective wage margin over production including RND
v[32]=v[19]*(1-v[15])*(1-v[13])-((v[90]/v[7])+v[11]*v[19]);												//sector effective profit margin over production
...
v[38]=v[36]/(1-v[36]);																			//sector effective direct and indirect input tech coefficient
...
CYCLE(cur, "CLASSES")
{
	...
	v[55]=v[6]*v[51];																			//class initial autonomous consumption
		for (v[104]=1 ; v[104]<=v[2] ; v[104]=v[104]+1)                          				//for (class_period) lags
			...
			WRITELLS(cur, "Class_Financial_Obligations", 0, 0, v[104]);
			}
			WRITELLS(cur, "Class_Avg_Nominal_Income", v[54], 0, 1);
			WRITELLS(cur, "Class_Avg_Real_Income", (v[54]/v[19]), 0, 1);
			WRITELLS(cur, "Class_Real_Autonomous_Consumption", v[55], 0, 1);         				//write class' autonomous consumption
...
for (v[105]=1 ; v[105]<=v[4] ; v[105]=v[105]+1)		              								//for (government_period) lags	
	WRITELLS(cur,"Government_Debt", 0, 0, v[105]);                  							//no debt initially																	//base interest rate parameter
...
CYCLE(cur, "SECTORS")
{
	...
	v[63]=((v[9]/v[8])-v[11]*v[9])/v[7];                        								//sector initial wage
	...
	for (v[107]=1; v[107]<=v[2]; v[107]=v[107]+1)
		WRITELLS(cur, "Sector_Avg_Price", v[19], 0, v[107]);                                   	//Avg_Price equals avg_price initial
	for (v[108]=1 ; v[108]<=(v[2]+1) ; v[108]=v[108]+1)                        		 			//for (class_period+1) lags
		...
		if(v[58]==1)
			WRITELLS(cur, "Sector_Productive_Capacity", (((v[41]+v[43])*v[76]/v[0])/(1-v[76]/v[0])), 0, 1);			//All firms start operating at desired degree of utilization, thus, productive capacity is endogenous calculated based on effective production and desired degree
		else
			WRITELLS(cur, "Sector_Productive_Capacity", ((v[60]*(1+v[17]))/v[18]), 0, 1);			//All firms start operating at desired degree of utilization, thus, productive capacity is endogenous calculated based on effective production and desired degreeWRITELLS(cur, "Sector_Exports", (v[49]/3), 0, 1);										//Total exports are divided equally among sectors.
		...
		WRITES(cur, "sector_initial_price", v[19]);
	...
	for (v[109]=1 ; v[109]<=v[0] ; v[109]=v[109]+1)                                				//for (investment_period) lags
		...
		WRITELLS(cur1, "Firm_Capacity_Utilization", v[18], 0, v[109]);}
	...
	for (v[112]=1 ; v[112]<=(v[1]-1) ; v[112]=v[112]+1)											//for (markup_period-1) lags
		...
		WRITELLS(cur1, "Firm_Effective_Market_Share", (1/v[6]), 0, 1);                    		//firm's effective market share will be the initial market share       
	  	...
	  	if(v[58]==1)
			WRITELLS(cur1, "Firm_Productive_Capacity", (((v[41]+v[43])*v[76]/v[0])/(1-v[76]/v[0]))/v[6], 0, 1);			//firm productive capacity will be the sales divided by the desired degree of capacity utlization (parameter)
		else
	  	...
	  	WRITELLS(cur1, "Firm_Wage", v[90], 0, 1); 												//firm's nominal wage equals sector nominal wage initial
		WRITELLS(cur1, "Firm_Variable_Cost", ((v[90]/v[7])+v[11]*v[19]), 0, 1);							//firm variable cost equals unitary wage plus unitary cost of inputs. The last equals the tech coefficient if input price equals 1
		...
		WRITELLS(cur1, "Firm_Stock_Deposits", 0, 0, 1);									//no financial assets initially
	 	...
	 	CYCLES(cur, cur1, "FIRMS")                                                 				//CYCLE trough all firms
			{
			v[70]=SEARCH_INSTS(root, cur1);														//search current firm position in the total economy
			...
			for(v[114]=1; v[114]<=v[62]; v[114]=v[114]+1)                        				//for the number of capital goods of each firm
			{
